...
export const description = "Cancel a held or confirmed booking. Releases the time slot.";

export const inputSchema = {
  bookingId: z.string().describe("The booking ID to cancel."),
  reason: z.string().optional().describe("Optional cancellation reason for record-keeping."),
...
export async function handler(
  ...
  if (args.reason) {
    ...
    console.info(`[floyd-mcp] cancel reason for ${args.bookingId}: ${args.reason}`);
  }

  try {
    const response = await client.cancelBooking(args.bookingId, args.idempotencyKey);
